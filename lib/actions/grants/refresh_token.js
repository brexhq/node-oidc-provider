...
module.exports.handler = function getRefreshTokenHandler(provider) {
  ...
  return async function refreshTokenResponse(ctx, next) {
    ...
    if (refreshTokenRotation === 'rotateAndConsume') {
      try {
        gracePeriod = conf.refreshTokenGracePeriod[refreshToken.clientId] || conf.refreshTokenGracePeriod['default']
        if (
          refreshToken.consumed
          && refreshToken.consumed + conf.refreshTokenGracePeriod < getEpochTime()
        ) {
        ...
        if (!conf.refreshTokenSoftDelete) {
