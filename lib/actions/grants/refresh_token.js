...
module.exports.handler = function getRefreshTokenHandler(provider) {
  ...
  return async function refreshTokenResponse(ctx, next) {
    ...
    const {
      refreshTokenGracePeriod, refreshTokenGracePeriodDefault, refreshTokenRotation, audiences, features: { conformIdTokenClaims },
    } = conf;
    ...
    if ((ctx.oidc.client.refreshTokenStrategy || refreshTokenRotation) === 'rotateAndConsume') {
      try {
        if (refreshToken.consumed) {
          ...
          let gracePeriod = refreshTokenGracePeriod[refreshToken.clientId] || refreshTokenGracePeriodDefault
