...
module.exports.handler = function getRefreshTokenHandler(provider) {
  ...
  return async function refreshTokenResponse(ctx, next) {
    ...
    if ((ctx.oidc.client.refreshTokenStrategy || refreshTokenRotation) === 'rotateAndConsume') {
      try {
        if (refreshToken.consumed) {
          ...
          let gracePeriod = conf.refreshTokenGracePeriod[refreshToken.clientId] || conf.refreshTokenGracePeriodDefault
          if (getEpochTime() > refreshToken.consumed + gracePeriod) {
            throw new InvalidGrant(`refresh token already used. ${refreshToken.consumed} + ${gracePeriod} > ${getEpochTime()}. ${refreshToken.clientId} ${conf.refreshTokenGracePeriod[refreshToken.clientId]} ${JSON.stringify(conf.refreshTokenGracePeriod)}`);
          }
        }
