      ...
      try {
        if (refreshToken.consumed) {
          ...
          if (getEpochTime() > refreshToken.consumed + gracePeriod) {
            throw new InvalidGrant(`refresh token already used. ${refreshToken.consumed} + ${gracePeriod} > ${getEpochTime()}. ${refreshToken.clientId} ${conf.refreshTokenGracePeriod[refreshToken.clientId]} ${conf.refreshTokenGracePeriod}`);
          }
        }
