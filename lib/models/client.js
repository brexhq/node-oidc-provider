...
function compareRegexps(values, value) {
  for (const val of values) {
    if (val.startsWith('s/')) {
      const regex = new RegExp(val.substring(2));
...
module.exports = function getClient(provider) {
  ...
  class Client {
    ...
    redirectUriAllowed(redirectUri) {
      ...
      if (instance(provider).configuration('features.disableProductionRestrictions')) {
        return compareRegexps(this.redirectUris, redirectUri);
      } else {
    ...
    originAllowed(origin) {
      ...
      if (instance(provider).configuration('features.disableProductionRestrictions')) {
        return compareRegexps(origins, redirectUri);
      } else {
    ...
    postLogoutRedirectUriAllowed(uri) {
      if (instance(provider).configuration('features.disableProductionRestrictions')) {
        return compareRegexps(this.postLogoutRedirectUris, uri);
      } else {
