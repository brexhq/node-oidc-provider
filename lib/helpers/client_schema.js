...
module.exports = function getSchema(provider) {
  ...
  // RECOGNIZED_METADATA.push('access_token_ttl');
  // DEFAULT.access_token_ttl = 60 * 60;  // one hour in seconds

  // RECOGNIZED_METADATA.push('refresh_token_ttl');
  // DEFAULT.refresh_token_ttl = 14 * 24 * 60 * 60;  // two weeks in seconds

  RECOGNIZED_METADATA.push('refresh_token_grace_period')
  DEFAULT.refresh_token_grace_period = 5 * 60;  // five minutes in seconds

  RECOGNIZED_METADATA.push('refresh_token_strategy')
  DEFAULT.refresh_token_strategy = 'rotateAndConsume'
  ...
  class Schema {
    ...
    postLogoutRedirectUris()
      const disableProductionRestrictions = configuration.features.disableProductionRestrictions;

      if (!this.post_logout_redirect_uris) return;

      this.post_logout_redirect_uris.forEach((uri) => {
        // This is a regexp, no point in validating it
        if (disableProductionRestrictions && uri.startsWith('s/')) {
          return;
        }
    ...
    redirectUris() {
      ...
      this.redirect_uris.forEach((redirectUri) => {
        ...
        // This is a regexp, no point in validating it
        if (disableProductionRestrictions && redirectUri.startsWith('s/')) {
          return;
        }
        ...
        switch (this.application_type) { // eslint-disable-line default-case
          case 'web': {
            ...
            if (!disableProductionRestrictions && this.grant_types.includes('implicit') && protocol === 'http:') {
              invalidate('redirect_uris for web clients using implicit flow MUST only register URLs using the https scheme');
            }

            if (!disableProductionRestrictions && this.grant_types.includes('implicit') && hostname === 'localhost') {
            ...
            if (features.oauthNativeApps) {
              switch (protocol) {
                ...
                default: // Private-use URI Scheme Redirection
                  if (!disableProductionRestrictions && !protocol.includes('.')) {
